...
public class DeepML extends AbstractDeepNeuralNet implements TechnicalInformationHandler {
	...
	protected RBM createDBM(int d) throws Exception {
		return ((m_N == 1) ? 
			new RBM(this.getE_dbpnn(), this.getH_dbpnn(), this.getM_dbpnn(), this.getR_dbpnn()) :		// RBM
			new DBM(this.getE_dbpnn(), this.getH_dbpnn(), this.getM_dbpnn(), this.getR_dbpnn())) ;		// DBM
	}
