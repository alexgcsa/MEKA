 ...
 */
public class ARAMNetworkfast2 extends ARAMNetworkClass {
    ...
    if (!instance.classIsMissing()) {
     //Do the weight updates using the instance.
			...
			if(!resonance && currentSortedIndex>=cateacti.length)
			{
				// Add a new category
				for (int j = 0; j < data.length; j++) {
					weightsA[numCategories-1][j] = data[j];
				}

				for (int j = 0; j < weightsB[numCategories-1].length; j++) {
					weightsB[numCategories-1][j] = labels[j];
				}
				String s = Arrays.toString(labels);
				if (hmclasses.containsKey(s)){
					hmclasses.get(s).add(numCategories-1);
				hmclasses.put(s,hmclasses.get(s)); 
				}else{
					Vector v = new Vector();
					v.add(numCategories-1);
					hmclasses.put(s,v);
				}
				ARAMm_Add_New_Category();
				// fprintf(FileID,'Add a new category of %d\n',
...
//			sortedActivations[0].getValue()sortedActivations[i].getOriginalIndex()
	...
	private SortPair[] ARTActivateCategories(double[] Data, double[] labels) {
		...
		for (int i = 0; i < lclasses.size(); i++) {
			...
			for (int j = 0; j < numFeatures; j++) {
				matchVector[j] = ((Data[j] < weightsA[k][j]) ? Data[j]
						: weightsA[k][j]);
				sumvector += matchVector[j];
...
//  }
  ...
  public static void main(String [] argv) {
    ...
    try {
     meka.classifiers.multilabel.AbstractMultiLabelClassifier.evaluation(new WvARAM(), argv);
    } catch (Exception e) {
      e.printStackTrace();
